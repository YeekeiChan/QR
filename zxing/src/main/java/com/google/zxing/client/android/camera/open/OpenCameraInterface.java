...
public final class OpenCameraInterface {
  ...
  public static OpenCamera open(int cameraId) {
    ...
    int numCameras = Camera.getNumberOfCameras();//获取摄像头的数量
    //如果没有找到相机则退出
    if (numCameras == 0) {
    ...
    if (explicitRequest) {
      ...
      while (index < numCameras) {
        Camera.CameraInfo cameraInfo = new Camera.CameraInfo();//初始化相机信息类
        Camera.getCameraInfo(index, cameraInfo);//获取相机的信息
        CameraFacing reportedFacing = CameraFacing.values()[cameraInfo.facing];
        if (reportedFacing == CameraFacing.BACK) {//判断是否是后置摄像头
